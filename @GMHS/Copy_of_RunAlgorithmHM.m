function [cont,paretoFront,iter] = RunAlgorithmHM(gmhs,TP,acceptedError,seed)
rng(seed);
currentObj=1;
mode='decomposition';
...
calc=1-exp(numerate/dom);
  ...
  [gmhs,cont]=ImprovisationHM(gmhs,iter,cont,mode,currentObj);
  gmhs.HM=Evaluate(gmhs,gmhs.HM,mode,currentObj);
  gmhs= Selection3(gmhs,iter);
  gmhs=DetermineDomination(gmhs);
 gmhs=updateExtArchive(gmhs,gmhs.HM);
